...
static int KD11HasPendingIRQ(void);

void KD11Init(void)
{
	memset(&lsi.kd11, 0, sizeof(KD11));
}

void KD11Reset(void)
{
	lsi.kd11.r[7] = 0173000;
	lsi.kd11.psw = 0;
	lsi.kd11.trap = 0;
	lsi.kd11.state = STATE_HALT;
	lsi.kd11.odt.state = ODT_STATE_INIT;
}

#define	READ(addr)		(LSI11Read((addr)))
#define	WRITE(addr, val)	(LSI11Write((addr), (val)))
#define	CHECK()			{ \
	if((kd11->trap && kd11->trap <= 010) || lsi.bus.nxm) \
		return; \
	}

static void KD11ODTClear(void)
{
	lsi.kd11.odt.buf_sz = 0;
}

static void KD11ODTWrite(u8 c)
{
	lsi.kd11.odt.buf[lsi.kd11.odt.buf_sz++] = c;
	lsi.kd11.odt.buf_r = 0;
}

static void KD11ODTWriteOctal(u16 val)
{
	int i;
	lsi.kd11.odt.buf[lsi.kd11.odt.buf_sz++] = ((val >> 15) & 0x7) + '0';
	for(i = 0; i < 5; i++) {
		lsi.kd11.odt.buf[lsi.kd11.odt.buf_sz++] = ((val >> 12) & 7) + '0';
		val <<= 3;
	}
}

static void KD11ODTInputError(void)
{
	lsi.kd11.odt.state = ODT_STATE_WR;
	lsi.kd11.odt.next = ODT_STATE_WAIT;
	lsi.kd11.odt.buf[0] = '?';
	lsi.kd11.odt.buf[1] = '\r';
	lsi.kd11.odt.buf[2] = '\n';
	lsi.kd11.odt.buf[3] = '@';
	lsi.kd11.odt.buf_r = 0;
	lsi.kd11.odt.buf_sz = 4;
}
...
static inline u16 KD11CPUReadW(u16 dst, u16 mode, int inc)
{
	KD11* kd11 = &lsi.kd11;
	u16 addr;
	switch(mode) {
...
static inline u8 KD11CPUReadB(u16 dst, u16 mode, int inc)
{
	KD11* kd11 = &lsi.kd11;
	u16 addr;
	switch(mode) {
...
static inline void KD11CPUWriteW(u16 dst, u16 mode, u16 val)
{
	KD11* kd11 = &lsi.kd11;
	u16 addr;
	switch(mode) {
...
static inline void KD11CPUWriteB(u16 dst, u16 mode, u8 val)
{
	KD11* kd11 = &lsi.kd11;
	u16 addr;
	switch(mode) {
...
static inline u16 KD11CPUGetAddr(u16 dst, u16 mode)
{
	KD11* kd11 = &lsi.kd11;
	u16 addr;
	switch(mode) {
...
#define	CPUREADW(rn, mode)	KD11CPUReadW(rn, mode, 1); \
				CHECK()
#define	CPUREADB(rn, mode)	KD11CPUReadB(rn, mode, 1); \
				CHECK()
#define	CPUREADNW(rn, mode)	KD11CPUReadW(rn, mode, 0); \
				CHECK()
#define	CPUREADNB(rn, mode)	KD11CPUReadB(rn, mode, 0); \
				CHECK()
#define CPUWRITEW(rn, mode, v)	KD11CPUWriteW(rn, mode, v); \
				CHECK()
#define CPUWRITEB(rn, mode, v)	KD11CPUWriteB(rn, mode, v); \
				CHECK()
...
static inline s32 KD11FPURoundPack(KD11* kd11, s32 sign, s32 exp, s32 frac,
		unsigned int* trap)
{
	/* round */
...
static inline s32 KD11FPUADD(KD11* kd11, s32 ac, s32 src, unsigned int* trap)
{
	*trap = 0;
...
static inline s32 KD11FPUMUL(KD11* kd11, s32 ac, s32 src, unsigned int* trap)
{
	*trap = 0;
...
static inline s32 KD11FPUDIV(KD11* kd11, s32 ac, s32 src, unsigned int* trap)
{
	*trap = 0;
		...
		case 000: /* 00 xx xx group */
			switch(insn >> 6) {
				case 00000: /* 00 00 xx group */
					...
					break;
				case 00001: /* JMP */
					tmp = KD11CPUGetAddr(insn1->rn, insn1->mode);
					CHECK();
					kd11->r[7] = tmp;
					break;
				...
				case 00046:
				case 00047:
					dst = KD11CPUGetAddr(insnjsr->rn, insnjsr->mode);
